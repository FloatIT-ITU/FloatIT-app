...
class AttendeeListBuilder extends StatelessWidget {
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return LoadingWidgets.loadingIndicator(
              message: 'Loading attendees...', size: 40);
        }
        ...
        if (!snapshot.hasData || snapshot.data!.isEmpty) {
          return Text(
              title.isEmpty ? 'No attendees found.' : 'No $title found.');
        }

        final data = snapshot.data!;
        final publicUsers =
            data['publicUsers'] as Map<String, DocumentSnapshot>;
        final privateUsers =
            data['privateUsers'] as Map<String, DocumentSnapshot>;
        ...
        for (final uid in attendeeUids) {
          ...
          final color =
              _colorFromDynamic(userData[FirestoreConstants.iconColor]);
  ...
  static Future<Map<String, dynamic>> _fetchAttendeeData(
      List<String> uids, bool includePrivate) async {
    if (uids.isEmpty) {
      return {
    ...
    for (int i = 0; i < uids.length; i += 10) {
      ...
      publicBatches.add(FirebaseFirestore.instance
          .collection('public_users')
          .where(FieldPath.documentId, whereIn: batch)
          .get());
      if (includePrivate) {
        privateBatches.add(FirebaseFirestore.instance
            .collection('users')
            .where(FieldPath.documentId, whereIn: batch)
            .get());
      }
